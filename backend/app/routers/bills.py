...
def create_bill(
    body: BillCreate, db=Depends(get_db), _: UserOut = Depends(require_receptionist)
):
    cursor = db.cursor(cursor_factory=RealDictCursor)
...
def get_bill(
    bill_id: int, db=Depends(get_db), current_user: UserOut = Depends(get_current_user)
):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute(
...
def update_bill(
    ...
    try:
        ...
        cursor.execute(f"UPDATE bills SET {set_clauses} WHERE bill_id = %s", values)
        db.commit()
...
def delete_bill(
    bill_id: int, db=Depends(get_db), _: UserOut = Depends(require_receptionist)
):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute(
