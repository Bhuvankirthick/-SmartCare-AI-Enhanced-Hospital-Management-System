...
def create_appointment(
    ...
    try:
        ...
        conflict = _check_conflict(
            cursor, body.doctor_id, body.appointment_date, body.appointment_time
        )
        if conflict:
            raise HTTPException(
                status_code=409,
...
def get_appointment(
    appointment_id: int, db=Depends(get_db), _: UserOut = Depends(get_current_user)
):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute(
...
def update_appointment(
    ...
    try:
        cursor.execute(
            "SELECT * FROM appointments WHERE appointment_id = %s LIMIT 1",
            (appointment_id,),
        )
        appt = cursor.fetchone()
        if not appt:
        ...
        if (
            body.appointment_date and body.appointment_date != appt["appointment_date"]
        ) or (
            body.appointment_time and body.appointment_time != appt["appointment_time"]
        ):
            conflict = _check_conflict(
                cursor, appt["doctor_id"], new_date, new_time, exclude_id=appointment_id
            )
            if conflict:
                raise HTTPException(
                    status_code=409,
        ...
        cursor.execute(
            f"UPDATE appointments SET {set_clauses} WHERE appointment_id = %s", values
        )
        db.commit()
...
def delete_appointment(
    appointment_id: int, db=Depends(get_db), _: UserOut = Depends(require_any_staff)
):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute(
