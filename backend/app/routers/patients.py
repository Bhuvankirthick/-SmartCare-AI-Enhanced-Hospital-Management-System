...
def list_patients(skip: int = 0, limit: int = 100, q: str = None, db = Depends(get_db),
...
def create_patient(body: PatientCreate, db = Depends(get_db), _: UserOut = Depends(require_any_staff)):
    cursor = db.cursor(cursor_factory=RealDictCursor)
...
def get_patient(patient_id: int, db = Depends(get_db), current_user: UserOut = Depends(get_current_user)):
    if current_user.role == "patient" and current_user.linked_id != patient_id:
        raise HTTPException(status_code=403, detail="Access denied")
    
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("SELECT * FROM patients WHERE patient_id = %s LIMIT 1", (patient_id,))
        patient = cursor.fetchone()
    finally:
        cursor.close()
...
def update_patient(patient_id: int, body: PatientUpdate, db = Depends(get_db),
                   _: UserOut = Depends(require_any_staff)):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("SELECT * FROM patients WHERE patient_id = %s LIMIT 1", (patient_id,))
