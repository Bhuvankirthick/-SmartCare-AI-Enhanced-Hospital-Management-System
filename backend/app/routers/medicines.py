...
from ..auth.rbac import get_current_user, require_pharmacist
...
def list_medicines(skip: int = 0, limit: int = 100, db = Depends(get_db)):
    ...
    try:
        cursor.execute("SELECT * FROM medicines ORDER BY medicine_id OFFSET %s LIMIT %s", (skip, limit))
...
def create_medicine(body: MedicineCreate, db = Depends(get_db), _: UserOut = Depends(require_pharmacist)):
    cursor = db.cursor(cursor_factory=RealDictCursor)
...
def get_medicine(medicine_id: int, db = Depends(get_db)):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("SELECT * FROM medicines WHERE medicine_id = %s LIMIT 1", (medicine_id,))
        med = cursor.fetchone()
    finally:
        cursor.close()
...
def update_medicine(medicine_id: int, body: MedicineUpdate, db = Depends(get_db),
...
def delete_medicine(medicine_id: int, db = Depends(get_db), _: UserOut = Depends(require_pharmacist)):
    cursor = db.cursor(cursor_factory=RealDictCursor)
    try:
        cursor.execute("DELETE FROM medicines WHERE medicine_id = %s RETURNING medicine_id", (medicine_id,))
        if not cursor.fetchone():
            raise HTTPException(status_code=404, detail="Medicine not found")
        db.commit()
    except Exception as e:
